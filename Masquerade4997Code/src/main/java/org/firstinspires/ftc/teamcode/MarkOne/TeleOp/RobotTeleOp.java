...
public class RobotTeleOp extends MasqLinearOpMode {
    ...
    public void runLinearOpMode() throws InterruptedException {
        ...
        while(opModeIsActive()) {
            ...
            else robot.MECH(controller1,Math.sqrt(2) * 0.9, 0.5);
            ...
            if (controller2.leftBumper()) robot.lift.setVelocity(controller2.leftStickY()*0.5);
            else robot.lift.setVelocity(controller2.leftStickY());

            robot.tapeMeasure.setPower(controller2.rightStickY());

            if (controller2.a()) robot.blockGrabber.setPosition(0.007);
            else if (controller2.x()) robot.blockGrabber.setPosition(0.97);
