...
import com.disnodeteam.masqcv.filters.MasqCVColorFilter;
import com.disnodeteam.masqcv.filters.MasqColorFilter;
import com.disnodeteam.masqcv.math.Line;
import com.disnodeteam.masqcv.math.Lines;
import com.disnodeteam.masqcv.math.MathFTC;
import com.disnodeteam.masqcv.math.Points;
...
public class CryptoboxDetector extends OpenCVPipeline {
    ...
    public MasqCVColorFilter colorFilterRed     = new MasqColorFilter(MasqColorFilter.ColorPreset.RED);
    public MasqCVColorFilter colorFilterBlue    = new MasqColorFilter(MasqColorFilter.ColorPreset.BLUE);
    ...
    public Mat processFrame(Mat rgba, Mat gray) {
       ...
       /* for (int i = 0; i < groupings.size(); i++) {
        ...
        if(newFullLine.length() > 75){
            trackables = new ArrayList<>();
            Log.d("MasqCV", "RESETTING TRACKABLE!");
        }
        fullAvgPoint = newFull;
        //  Imgproc.cvtColor(white, white, Imgproc.COLOR_RGB2HSV);


        Imgproc.putText(rgba,"MasqCV 1.1 Crypto: " + newSize.toString() + " - " + speed.toString() + " - " + detectionMode.toString() ,new Point(5,30),0,1.2,new Scalar(0,255,255),2);
