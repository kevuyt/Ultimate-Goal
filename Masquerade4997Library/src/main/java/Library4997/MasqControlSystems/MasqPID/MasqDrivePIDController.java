 ...
 */
public class MasqDrivePIDController {
    private MasqIntegrator integrator = new MasqIntegrator();
    private double kp, ki = 0, kd = 0, prevError = 0, prevD = 0, prevTime;
    private double deriv, timeChange = 0;
    private double error, current, target;
    private MasqClock clock = new MasqClock();

    public MasqDrivePIDController(double kp, double ki, double kd) {
        this.kp = kp;
        this.ki = ki;
        this.kd = kd;
    }
    public MasqDrivePIDController(double kp, double ki) {
        this.kp = kp;
        this.ki = ki;
    }
    public MasqDrivePIDController(double kp) {
        this.kp = kp;
    }
    ...
    public double getOutput (double error, double timeChange) {
        ...
        prevError = error;
        prevD = deriv;
        prevTime = clock.milliseconds()/1e3;
    ...
    public double getOutput (double error) {
        return getOutput(error,clock.milliseconds()/1e3 - prevTime);
    }
    public double getKp() {
        return kp;
    }
